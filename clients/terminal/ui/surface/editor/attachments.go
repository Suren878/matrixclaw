...
type Keymap struct {
	DeleteMode key.Binding
	DeleteAll  key.Binding
	Escape     key.Binding
}

func NewAttachments(renderer *Renderer, keyMap Keymap) *Attachments {
	return &Attachments{
		keyMap:   keyMap,
		renderer: renderer,
	}
...
func (m *Attachments) List() []Attachment {
	return append([]Attachment(nil), m.list...)
}
...
func (r *Renderer) Render(attachments []Attachment, deleting bool, width int) string {
	var chips []string
...
func (r *Renderer) icon(a Attachment) lipgloss.Style {
	if a.IsImage() {
		return r.imageStyle
	}
	return r.textStyle
