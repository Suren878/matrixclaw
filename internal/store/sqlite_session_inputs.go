...
INSERT INTO session_inputs(id, session_id, target_run_id, mode, status, text, parts_json, client, external_key, working_dir, consumed_run_id, error, created_at, updated_at, consumed_at)
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		input.ID,
		input.SessionID,
		input.TargetRunID,
...
SET target_run_id = ?, mode = ?, status = ?, text = ?, parts_json = ?, client = ?, external_key = ?, working_dir = ?, consumed_run_id = ?, error = ?, updated_at = ?, consumed_at = ?
WHERE id = ?`,
		input.TargetRunID,
		string(input.Mode),
		string(input.Status),
...
func (s *SQLiteStore) ListPendingSessionInputs(ctx context.Context, sessionID string) ([]core.SessionInput, error) {
	query := `
SELECT id, session_id, target_run_id, mode, status, text, parts_json, client, external_key, working_dir, consumed_run_id, error, created_at, updated_at, consumed_at
FROM session_inputs
WHERE status = ?`
